...
- **档位分档（B2）**：**基准档 = Deep**（上表与对外数字均按 Deep）；`Light` 档（跳深验面）实测省幅 **n3v2 ≈19–21%**（合成 10k · 同进程交替 A/B · 带不交叠，两复跑）、**nquads ≈12.5%**（同法）、**trig ≈19–21%**（三次复跑，后两次判带不交叠；首轮因 light 一记离群判交叠，已留痕）。**默认仍为 Deep**：性能不换深验；`Light` 需调用方显式选择（口径见 `src/gen_nquads/const.md` §X）。
