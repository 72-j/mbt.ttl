...
经工具链 `moon fmt` 后与 check-in 产物**逐字节**相等且强幂等（`n3.mbt` 由 `src/rdf/n3gen` 的 G9 门把关）。
  ...
  **不代表 W3C 认证、合规认定或任何形式的背书**；W3C 名称与商标不得用于本项目的广告或宣传。
  ...
  ```
   实现                  解析时间    每秒解析     每条耗时
  ━━━━━━━━━━━━━━━━━━━━  ━━━━━━━━━━  ━━━━━━━━━━━  ━━━━━━━━━━━
   MoonTTL (MoonBit)     0.56 ms     1,786,445    0.00056 ms
  ────────────────────  ──────────  ───────────  ───────────
   Oxigraph (Rust)       0.77 ms     1,294,934    0.00077 ms
  ────────────────────  ──────────  ───────────  ───────────
...
- **同机同档实测（2026-09-25，本机 Ryzen 7 5700G，`--target native --release`，同为 in-process 只量解析面）**：本实现 **0.00057 ms/三元**（10k 档 5.697 ms）vs **Oxigraph 0.5.11 0.00077 ms/三元**（10k 档 7.689 ms）⇒ **≈1.35×**。**读法要紧**：对端**只解析**，本实现是**解析 + 四门深验 + 物化（不建图）**——即本实现做更多事仍更快。旧叙事"≈3.3×–3.5×"基于 Oxigraph **公开读数 0.0020**（500k 语料 / 异机），本机实测对端比它快 2.6×，故该公开读数**不再作对端代表值**；复现条件（命令 / 探针件 / 冷缓存 / 机指纹）见 `perf-review.txt`。
- **档位分档（B2）**：**基准档 = Deep**（上表与对外数字均按 Deep）；`Light` 档（**关两把开关：组装层轻验 + 物化层四门深验**，逐方言同名不同义见 `const.md` §X 第 1 条）实测省幅 **n3v2 ≈19–21%**（合成 10k · 同进程交替 A/B · 带不交叠，两复跑）、**nquads ≈12.5%**（同法）、**trig ≈19–21%**（三次复跑，后两次判带不交叠；首轮因 light 一记离群判交叠，已留痕）。**默认仍为 Deep**：性能不换深验；`Light` 需调用方显式选择（口径见 `src/gen_nquads/const.md` §X）。

```moonbit
// 严格（默认，不变）
let slice = @nquads.SliceParser::new(engine, data)
let (quads, errs) = slice.parse_all()
let (emits, merrs) = mat.materialize_all(quads)          // 默认 Deep
