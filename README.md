...
- **对端为公开读数、非同口径**：Oxigraph 0.0020 ms/三元（本 README 上表；500k 语料、异机，且其建图而本实现解析 + 四门深验 + 物化不建图）⇒ **同量纲换算 ≈3.5× 量级参照**（2026-09-25 复测：本实现 0.00057 ms/三元，冷缓存 native release，机指纹见 `perf-review.txt`）。
