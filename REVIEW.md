...
| 子仓 | **数据面 ⟷ 引擎 值级对拍** | 快照行 ∩ 可达态，逐槽等值（含**帧面**：帧栈/倒装两槽）；不可比逐类计数钉死 | **n3v2 314 行 / 可比 314 / 违例 0 / 债点 0 / 不可比 15（全为 `pop_writes` 立项族；面外 0）；trig 136 行 / 可比 136 / 违例 0 / 债点 0 / 不可比 0**（§5①） |
| 子仓 | **快照格式所有权** | 产出侧：任一字段含列分隔符即 `panic`；消费侧：列数恰 7、行数钉子 | **format v3（列分隔符 = TAB；帧写落位写实 `frame.top./below./popped.`）** |
| 子仓 | 测试 | 本机 | **438/438** |
