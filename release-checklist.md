...
## 网络口径（2026-09-26 实测，用户确认）

- **`mooncakes.io`（注册表 / publish 验收）**：**直连即可**，**不需要代理**（实测 `api/v0/search?kw=moonttl` 直连返回 0.2.2；走 `socks5://127.0.0.1:1080` 反而空响）。
- **GitHub（`api.github.com` / Actions 观测）**：**偶尔需要代理**（本机 2026-09-25 直连成功、2026-09-26 走 socks5 失败 ⇒ 两条路时通时不通，**遇阻再挂 `socks5://127.0.0.1:1080`**）。
- **gitlink（推送）**：SSH 直连（本仓 `origin`）——历次推送未用代理。
⇒ **发版日判据相关链路（注册表回查）不依赖代理**；只有 GitHub 侧观测/触发偶尔需要。
