...
1. **R-15 落 gen_nquads 而非账面建议的 n3v2 types.mbt**——侦察实证 trig
   `lexer_adapter.mbt` 曾有 `is_numeric_span` 字节孪生件，验收"has_digit 全仓一处"
...
## ADR-35：校验层升格进主判定链（R-16 step4，R-07 题6 的后续）——✅ 2026-09-13

**背景**：役25（R-07 题6=A）把三 runner 做成**双判**——主链宽容（`lenient=true`）、影子严格，
影子缺口冻结为基线（5/0/123/13）。R-16 step1–3 修口后缺口 **488 → 0**，升格的前置条件（"清零后升格"）
达成。

**裁决**：**主判定链改走严格组装校验**（`N3SliceParser::new(engine, sec)`，`lenient` 缺省 false）；
原"影子"改为**反向哨兵**（宽容口径重跑，结构上不应比严格多报，钉 0）。三 runner 同步：
`rdf_suite_wbtest` / `n3tests_suite_wbtest` / `examples_wbtest`。

**依据（零漂移）**：升格后所有判据数字与升格前**逐项相同**——
rdf-turtle **316/316**、rdf12-turtle **75/75**、N3Tests neg **23ok/0miss** + pos+eval **205clean/0mat-only/0parse-fail**、
examples **A=13/B=0/C=0**；三 runner 的 `strict-gap` 均为 **0**。这同时是"修口正确性"的最强证据：
严格面**没有比宽容面多报任何一例**。

**后果**：
- 套件数字首次由**校验层**产出（此前 `lenient` 跳过校验）；C-07 的"316/75 不覆盖校验层"欠账**清偿**。
- 反向哨兵保留一周期：若将来某人给 `lenient` 加了额外报错，哨兵会红（结构上不应发生）。
- 内建前缀表（`log:` 等）确认**非必需**（缺口清零与它无关），列为选做；`R-16` 全项收官。

## ADR-36：数据面值级对拍链回灌（§AN.9–15 + trig §AN.5）——✅ 2026-09-24

**背景**：quicktest 数据面值级对拍门（`quicktest/data_face.mbt`）注释里的决策线（§AN 系列）
定义于**活账**（主账 AN.5–AN.15，2026-09-19/20），入库卷此前无落点 ⇒ §AN 引用是悬空锚，
直接压注释 = 删证据链。本 ADR 回灌决策本体；过程实测与门况数字留活账，按号可查。

**裁断（七条）**：

1. **实参绑定是数据面的一部分**（trig §AN.5 裁定，§AN.7 回灌 n3v2）：`argN` 必须按该行
   `args` 列解引用——`payload`/`Some(payload)`→载荷、`none`→None、`ctx.<槽>`→该槽前值、
   余者逐项入册。不看 args 列 = 把"无实参"当"有实参"（门假阳实证）。
2. **清-读同槽按"读前值"真判；分派族按 `when_slot_type` 真判**（§AN.9–11）：整行跳过已废
   （"整行入册 ⇒ 行内可比槽失去判据"，§AN.1 教训）；分派族最终值由行实参 `SlotType` 定
   （引擎 `open_slot` 落位实证，扁平表"看似对实则错"）；数据面以 `QuickArm{key, writes}` 表达
   （键位：`SlotType` 名 / 行 `to` 态名），生成产物零迁移。
3. **快照格式 v2 = TAB 分隔符**（§AN.12）：v1 `|` 撞写槽词表 `fwd:a|b` ⇒ 静默截断 +
   **静默漏比**（最坏失效形态：门全绿而覆盖率虚高）。纪律：**格式的"无歧义"断言必须与词表
   增长同笔维护**——产出侧所有权断言（任一字段含 TAB 即 panic）+ 消费侧列数恰 7 + 行数钉子；
   版本号只留 `# format: vN` 注释（§AN.14：裸版本行会被行数统计误食）。
4. **条件式转真判**（§AN.12/§AN.15）：`fwd:` 按行进前方向旗（`SlotView.path_fwd`）二选一后
   递归同表解析；`nfwd:` 的翻后旗是编译期常量 ⇒ 投影侧坍缩成定值；
   `flag:<位>?<来源>` 条件编码真支递归同词表、假支 = 引擎不写 ⇒ 期望 = 行进前当前值。
5. **缺省绑定写实**（§AN.12）：行未声明 `action_args` 而写槽引用 `argN` 时，投影写实
   `payload`/`keep`；`keep` 的期望 = 行进前该槽值（一处写实，两门免复述）。
6. **帧面投影写实**（§AN.13–14，格式 v3）：`frame.<位>` 快照写实成**后态地址**
   `frame.top/below/popped`；弹帧行 `writes` 声明的是弹帧前**中间态**（语义上不可观测）⇒
   `pop_writes` 按行 `to` 态名声明**行终态**（键位选引擎真正分派所依据的行级数据），
   投影剔除 `frame.popped.*`（被弹帧后态不存在 ⇒ 值级面不可读），`scope_reset` 不再叠加
   （免二次清槽打回回装值）。
7. **两轴各自锚引擎**（§AN.11 + R-20）：数据面轴（声明 ⟷ 引擎）由本门管、模型轴
   （生成 ⟷ 引擎）由 R-20 普查管——drift 双向可抓；`SlotView` 不得随意加字段
   （普查的整块 `==` 尺，加字段即污染）。

**后果**：台账四类（面外/序敏感/条件式/arg未解析）后三类钉 0、面外归零 ⇒ 两方言值级对拍
100% 可比（n3v2 314 · trig 136 行逐槽有判据）；棘轮判据 = 类计数任一变化即红（能抓类间搬家，
强于只钉总数）。
