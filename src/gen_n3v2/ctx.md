...
- 声明 / 实现锚点：`n3.mbt:2069`（trait）/ `n3.mbt:2153`（`impl N3EffectHandler with fn interpret`）；
...
- 落地（役33，2026-09-22 收账）：step1–4 提交 `5ff9755` + `10a6434`；验收复跑实测——影子四桶 **0/0/0/0**（examples / N3Tests / rdf-turtle / rdf12-turtle），nquads 142/142、trig 357/316/36/75、n3v2 122/122 全绿（数字较立案时点增长为后续役新增套件，全绿口径不变）；`good_prefix.n3` 已出 STRICT-GAP 名单；判定差 = 0 以更强形式兑现——校验层本身升格主判定链（影子/主判同源，判据零漂移）；**路线翻案**：内建前缀表预绑定未做（strict-gap 0 实证不需要）；反例钉保留——拼错前缀（colon>0）仍必须拒。
...
| 主题 | 锚点 |
|---|---|
| 契约成员声明 | `n3.mbt:282`（trait 名 `N3Supervisor`，旧名 `N3LoopPolicy`）、`:299`（Actions）、`:2069`（EffectHandler） |
