...
| R-14 | 状态爆炸治理：子 FSM 自动组合交叉状态，或混合架构 | [立案] | 役30 |
...
### 役30 状态爆炸治理（R-14）— P2，前置：外层仓生成器立项（**2026-09-12 立项勘察，拆分 30a–30f**）
...
**待裁**：题7 = A/B/C 选型（建议：先 30a+30b 拿数据再裁，**C 兜底**）；题8 = 探针族取 `Quant`（建议）还是 `Annot`；
题9 = 是否以"产物字节等价"为唯一判据（建议：是）；题10 = 可达性门是否常设（建议：是）。

**验收（役收）**：新增一个特性只增"子机声明 + 接线"、**零手列交叉态**；G1–G11 绿；G9 字节等价贯穿 30c/30d；
模块 329/329、三套件数字不变；0 warning；`moon fmt` 幂等。
...
**验收**：门绿；对现状表"警告级"扫描零假报；把任一锚点登记删掉 → 门红（证明锚点源真的在生效）；
`cp` 交付后子仓 `moon test src/gen_n3v2` 116/116 不变。
...
（役22/25/23/24/26/27a/28/29 ✅ 2026-09-11；役30 已立项勘察并拆分 2026-09-12；R-04 长期半与 R-16 [立案] 另役）。
