...
## ADR-NQ-011：**词法四决策回灌**（加宽口径 / 单 `^` 门 / 右括族停扫 / 数值与布尔判定件）——✅ 2026-09-24（回灌）

**背景**：`lexer_mbt.mbt`（`Lexermoon`）的注释里长期挂着四类**词法决策**，但它们的出处只在
**活账**（`todo.md` 等，不入库）；入库卷（本文件 / `spec.md` / `const.md`）**查不到**
（2026-09-24 实测：`trim_trailing_punct` · `is_numeric_span` · `is_boolean_word` 在入库卷命中 **0**）。
⇒ 注释一旦"压行"就会**删证据链**。本 ADR 先把决策**回灌入库**，作为注释分层（层 2 压一行）的前置。

**决策（四条，回灌自 `lexer_mbt.mbt` 注释原文与可核事实；不发明历史）**

1. **词法加宽口径**（`spec.md` 的"加宽"落点同源）：token **全词含定界符**；未闭合 IRI/字面量**宽容出词**；
   `PrefName` 无脑扫（`:` / 内部 `.` / `\X` 转义对 / `%HH` / 多字节并入，**尾部裸 `.` 回退**）；
   **语法对错归验证层**。对齐源 = **C 版 `lexerc_ffi.c`**（承 **ADR-NQ-003**）。
2. **单 `^` 门**：字面量后缀只认 **`^^`**（datatype）与 **`@lang`**；**单个 `^` 不是后缀**，立即收束成独立 token，
   由引擎路径机 desugar（路径族口径见 `const.md`）。根因（注释原文）：旧门把 `"lit"^:prop` 的 `^` 误当 `^^`，
   连 `:prop` 胶进词尾 ⇒ **引擎收不到 Caret 事件**（N3Tests `path2` / `caret_pos` 双残差根因）。
3. **右括族停扫**：后缀合法域 = IRIREF / PrefName / langtag ⇒ 遇 **`)` `]` `}` 停止扫描**；
   而 **`.` `,` `;` 仍并入词尾**，由**引擎侧尾标点补偿 + 合成 Dot**承接（同族实现见 `src/gen_n3v2/engine.mbt`
   的 `trim_trailing_punct`）。根因（注释原文）：旧扫描把 `"a"^^xsd:int)` 的闭括号吞进词项 ⇒ 集合元素位失配。
4. **数值 / 布尔判定件共享**：`@nquads.is_numeric_span`（数值族，含 `±` 符号与前导点；**整词形**裁决）与
   `is_boolean_word`（小写精确 `true`/`false`）是**独此一份的识别件**，词法层与验证层**同源复用**
   （数值/布尔在词法层降 `Unknown`，**物化层**展开 typed literal / `xsd:boolean`）。

**被否方案**：① 只把四类决策"翻译成大白话"继续留在注释里（**复制而非分层**：同一事实两处，必然漂移）；
② 直接压注释、不回灌（= 删证据链，活账不入库 ⇒ 外部读者永久丢失"为什么"）。

**后果**：`lexer_mbt.mbt` 的层 2（施工账）可安全压成**一行 + 指向本 ADR**；层 1（领域语义）保留并补强；
层 3 只留**稳定符号**（ADR 号，不留役号/行号）；层 4（役号沿革）删除。

**回灌声明**：本条目由代码注释 + 可核事实**回灌**（2026-09-24 发版准备役 · 注释分层 B 案前置），
**不是**当时的原始裁定记录；若日后找到原始账页，以账页为准并同笔修订本条目。
