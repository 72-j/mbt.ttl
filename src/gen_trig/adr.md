...
**落点**：`spec.md` §5.5；`materialize_trig.mbt:351`（`base_at`）。
...
## ADR-TRIG-018：役35 35-B1 **三处裁定**（口径选择 / 有意分叉 / 不译语义）——✅ 2026-09-22

**背景**：`@keywords` 表源化（`KeywordX` 事件 + `ExpectKeywordsList` 态 + 11 行 + 台账/合成）落地时，
实现撞上三处"照 n3v2 直译"与"trig 自有口径"的张力点；用户 2026-09-22 裁定**三处判断都不推翻**，
但要求**写清定性、分叉层、边界与重评估条件**（本 ADR 即该要求的落点；结构事实见 `spec.md` §7.2）。

**决策（三条）**

1. **`a` 在台账在册即"复活" = 口径选择，不是规范强制**。`@keywords a` 声明后 `a` 仍是 turtle
   `rdf:type` 简写（照 n3v2 `kw_a_live` 支）。判据载体 = 产物 `pk`（`KwA` vs `Normal`）。
   **重评估条件/改法**：若要"恒失效"（不论是否在册），删去 `engine.mbt normalize_term_span`
   的 `TrigKeywordA` 支末一条件 `!self.kw_ledger_hits(s2)`，同笔改钉 4 第 ③ 支的 `pk` 期望。
2. **组装层放行无冒号裸名 = 有意分叉**（分叉层 = **组装/轻验层** `parser_slice.mbt` 的 `prefix_declared`）。
   原因：listed 词到该层是**裸名 span**，旧口径报 `Undeclared prefix`（实测钉 8/9 红）⇒ 采
   "**合法性归表，组装层不二次裁决**"（§AS.8 ⑤"照 `PrefName` 径"）。
   **边界**：只有**引擎收下的词**才到得了本层（TriG/Turtle 裸词被适配层降 `Unknown`、N3 未列裸词被
   表外拒）⇒ **不放宽被接受的语言**。**"组装层放行 + 物化层严格"是有意分层，不是口径漂移**：
   物化层裸名按空前缀展开，默认前缀未声明 ⇒ `Ok(None)` ⇒ 槽位错误面。
   重评估条件：n3v2 组装层同步放行 ⇒ 本项降为同源项。
3. **两行"不译"（`ExpectKeywordsList + Comma`、`ExpectIsProp + KeywordX`）= 表源无此行 ⇒ `UnexpectedEvent`**。
   与"**显式拒绝**"的区别：前者是**没有行**、由兜底臂拒（`ExpectIsProp` 态在 trig 根本不存在）；
   后者指"表上有行且该行拒"——本处**无拒绝行可指**。
   **补法**：加 `ExpectKeywordsList + Comma`（无 action）一行 ⇒ IR 钉 **205→206→207** + 黄金门交付 +
   quicktest valid/臂成对补行；`ExpectIsProp` 行**不建议补**（trig 无 `Is` 族词行）。

**被否方案**：① 让组装层"识别 listed 词"再放行（需把台账/pk 透到轻验层——把表决策复制到组装层，
违"单一裁决者"）；② 把裸名展开逻辑上移组装层（让轻验承担物化职责）；③ 为 `Comma` 行顺手补行
（会与"列表不用逗号"的 trig 口径冲突，且属**扩张**而非本役范围）。

**后果**：`@keywords` 族在 trig 侧**表源可判、层间各守其职**：词性/合法性 = 表；裸名解释 = 组装层
放行 + 物化层展开/报错；三处张力点各有成文定性，后续改动有据（改哪一处、动哪些钉、IR 计数怎么变）。
