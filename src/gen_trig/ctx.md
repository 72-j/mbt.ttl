...
| 错误分类 / 容灾决策 | ③ `TrigLoopPolicy::recover` / `on_business_failed` | 可以（策略现写死在 `gen_trig/engine.mbt:240` 起） |
...
| 词 | 含义（本包语境） |
|---|---|
| `Hooks` | `TrigActions` + `TrigEffectHandler` 的单载体实现（`gen_trig/actions.mbt:9`）——按命名 ADR 应改 `TrigActionsImpl` |
...
- 锚点：`trig.mbt:223`、`gen_trig/engine.mbt:227/240/314/344/353`（`TrigLoopPolicy`）；`gen_trig/actions.mbt:9` +
  `engine.mbt` 字段（`Hooks`）。
...
#### R-T13 `@keywords` 语义豁免 **✅ A1 已落 2026-09-22（甲案）；A2 待打**

- **定案（用户 2026-09-22 拍甲）**：`@keywords` 与 `a` 失效是**方言差异** ⇒ 由 **dialect 档**表达
  （不用隐式开关）：`TrigDialect` 加 **`N3`** 档；**甲案让"表源按档不同形"成立**（乙案的开关会破坏这一点）。
- **落法（A1，受理层）**：N3 档受理 `TrigKeywordsKw`（吞事件 + 归位语句起点 ⇒ 置 `kw_directive_seen`）；
  **谓词态**（表上 `KeywordA` 有行的 12 态）下按事件种类归 **`pk`**（`a` ⇒ `KwA`；失效 ⇒ `Normal`）；
  物化层**改读 `pk`**（原按载荷文本判 ⇒ "决策漂在物化层"的 (Ⅱ) 形态被消除）。
- **顺链修（本笔实测挖出）**：`parser_slice.mbt` 的"合成谓词归一"原写作"**非 `Normal` 即合成**" ⇒
  新族 `KwA` 会把 `a` 的 p 槽置 `(0,0)`（`骨架: 'a' 缩写走谓词位` 当场红）⇒ 改为**显式合成族**
  （`RDFFirst`/`RDFRest`）。**口径：pk 加族必须逐点检查"按 pk 分派"的产物链**（正是 pk 同形判据的意义）。
- **边界（钉死）**：`a` 失效**仅**在 `N3` 档 **∧** `@keywords` 已受理后；TriG/Turtle 档恒为 turtle
  `rdf:type` 简写（W3C `bad-n3-extras-07/13` 负例口径不变）。
- **验收（已达成）**：`gen_trig` **92/92**（5 钉：负例 pk∧物化 / 正例 pk / 跨语句持久 / W3C 档仍拒 / N3 档其余拒例仍拒）；
  **N3 档套件：无**（W3C 无 N3 档）⇒ 入册为覆盖缺口（面一）。**A2**（列表区层）补钉 4（多关键词均失效）+ 列表吞词。
- **依赖**：T10 ✅（产物黄金门）——已满足。
...
**役35 35-B1（`@keywords` 表源化）四坑（2026-09-22 实测）**

- **守卫收编 ≠ 灭门**：A1/A2 的"吞事件 + 列表区"守卫退役后，**档位门必须留一处**
  （`!(dialect is N3) && TrigKeywordsKw ⇒ recover`）——表行对三档同源，不挡则 Turtle 档
  W3C `bad-n3-extras-07/13` 负例翻正例（套件必红）。
- **组装层不再二次裁决裸词**：listed 词到组装层时是**裸名 span**，`prefix_declared` 的
  "no colon ⇒ false" 会报 `Undeclared prefix`（钉 8/9 首跑实测红）⇒ 该分支改判
  "裸名 = 空前缀本地名"放行（合法性已由表裁决），**物化层仍严格**（默认前缀未声明 ⇒ 错误面）。
- **`a` 复活边界**：`@keywords a` 在册 ⇒ `a` **保持** `KeywordA`（`rdf:type` 简写）——
  照 n3v2 `kw_a_live` 支；A2 钉 4 "两词均失效"的口径**同笔改**（§AS.8 钉 12 授权"退役回归"）。
- **quicktest 是手维护实例**：新增状态/事件需"再生舞"（整目录移出 → 生成 → 与生成件逐臂
  手合并 → 删残留），且**全部计数面同笔动**：36→**37** 态 / 34→**35** 命令 / 1224→**1295** 对 /
  快照行 136→**146** / 可达命令名 25→**28**（不可达 9→**7**）。

- **三处定性别混**（ADR-TRIG-018 / `spec.md` §7.2）：① `a` 在册复活 = **口径选择**（改法：删一个合取项）；
  ② 组装层放行裸名 = **有意分叉**（分叉层 = 轻验层；"组装放行 + 物化严格"是**有意分层**，不是漂移）；
  ③ 两行不译 = **表源无行 ⇒ `UnexpectedEvent`**（不是显式拒绝；补法 = 加 `Comma` 行 ⇒ IR 钉 205→206→207）。
- **余项划界**：当前只放行 **listed 词到组装层的裸名**，**不是所有裸词**——更宽裸词语义另立案
  （别在 `@keywords` 族里顺手放宽 `validate_term` / 物化默认前缀）。
