...
**当前总状态（2026-09-20 复核刷新·二）**：T10–T17 中 **T14 / T15 / T16 / T17 已收官**；T10/T11/T12/T13 待开工。
基线：0 warning · 四套件 **357/316/36/75**（deferred 全零；= `suite-review.txt` 复核表）· `.mbti` 349 行 / 54 pub 行 ·
表 36 态 / 34 事件 / 10 效果 / 195 转移 · 内联 test **0**（T14 迁毕）· `.bak` **0**（T15 清毕）。
（旧行记于 2026-09-12：T16 已收官，T10–T15 待开工，内联 test 21 / `.bak` 5 —— 保留为沿革。）
...
| C-T5 | 包内死件 5 个 `.bak` | **已清·T15**（2026-09-20 复核：无 `.bak`） | R-T5 ✅ |
| C-T6 | 生产文件内联 test 21（materialize 16 + serialize 5） | **已清·T14**（2026-09-20 复核：两文件 0 个 `test`，已迁 `_wbtest.mbt`） | R-T6 ✅ |
| C-T7 | 卷面违规：`todo.md` 曾同时载宪法与规格 | **已收口·T16** | R-T7 ✅ |
| C-T8 | 与 gen_n3v2 装配层重复副本（同名 helper 交集 20 个） | **已收口·T17**（14 项纯函数抽 `gen_shared`；其余分叉入册 §7.1） | R-T8 ✅ |
| C-T9 | `moon.pkg:1` 头注漂移（仍写 gen_nquads） | **已修·T15**（2026-09-20 复核：现写 `gen_trig`） | R-T9 ✅ |
...
| R-T5 | 清包内死件（5 个 `.bak` 归档） | **✅ 已落地** | T15 |
| R-T6 | 测试归位（21 个内联 test 迁 `_wbtest.mbt`） | **✅ 已落地** | T14 |
| R-T7 | 卷面补全（拆 const/spec/adr，todo 只留账本） | **✅ 已落地** | T16 |
| R-T8 | 双包重复治理（抽共享件 或 有意分叉入 spec） | **✅ 混合制落地**（抽件 `gen_shared` + §7.1 裁定表） | T17 ✅ |
| R-T9 | `moon.pkg` 头注改实况 | **✅ 已落地** | T15（搭车） |
...
- 范围：`gen_trig` + `gen_n3v2`（同名 helper 交集 20 个；装配层 ≈4064 vs ≈4381 行并行副本）。
- step：① 出"共享面清单 + 抽件方案 + 分叉代价"评估；② 二选一：抽共享装配层件（新包/上移 `gen_nquads`）
...
| **P1-3** | 测试归位 | T14 | R-T6 | — | — |
| **P1-4** | 卷面补全 | T16 **✅** | R-T7 ✅ | — | — |
| **P2-1** | 双包重复治理 | T17 **✅** | R-T8 ✅ | T11–T13 | 题T3 = **混合制**（ADR-TRIG-017） |
| **P2-2** | 生成面修口（壳内后缀 / 三引号 / 词法性能 / 关键字豁免） | — [立案] | R-T10–R-T13 | T10（门先行） | — |

推荐执行序：**T10 →（T14 ∥ T15）→ T13 → T12 → T11 → ~~T17~~ ✅（2026-09-20，混合制）**；
...
| 2026-09-20 | **复核归位（非役）** | 台账实况对齐（随复核面建立，§6.5） | C-T5 / C-T6 / C-T9 三行台账与 §2.1 总状态行按**实况**刷新（T14/T15 落地的成果此前未入账）；R-T5 / R-T6 / R-T9 标 ✅ | 依据 = 2026-09-20 复核（`ls src/gen_trig/*.bak` 无匹配；`grep -c '^test '` 两文件 0）；旧行保留为沿革 |
| 2026-09-20 | **T17** | 双包重复治理（R-T8 / C-T8，题T3 = 混合制） | 新包 `src/gen_shared/`（4 文件 ≈455 行）：14 项纯函数逐字抽入（`deep_check_literal`/`deep_check_tt`/`literal_body_end`/`is_pn_local_esc`/`check_iri_view`/`check_bnode_view`/`tt_bool_word`/`is_scheme_byte`/`view_has_scheme`/`eq_ignore_case`/`eq_lower`/`at_style_kw`/`triple_term_inner_terms`/`slice_span`）；两包各一条**包级** `using @gs { … }`（零改调用点）；一处真分歧（`deep_check_literal` 的 `rdf12` 门控）取严者 | 验收：`moon check --deny-warn` 0 · `moon test` **473/473** · 四套件自报行**逐字节不变**（`suite-review.txt`）· 行覆盖基线 905 → **906‰**（同笔推高）；决策 ADR-TRIG-017；裁定表 §7.1 |
