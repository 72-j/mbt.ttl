...
版本：**v1.2.0**（2026-09-20；v1.0 = 立卷，v1.1 = 值级对拍收口存档 + 引用索引补模板/覆盖率纪律，
...
v1.2.0 = **建立复核面**（§1.1 复核表 + §1.2 缺口解释表；随世界宪法 v2.9 §6.5），见 §7）
...
| C-T8 | trig 与 n3v2 装配层 helper 重复（交集 20） | `[立案]` | 抽共享件 vs 有意分叉需裁决；两包当前各自演进 | 第三次同形修改出现时 | `src/gen_trig/spec.md` §8（T17） |
