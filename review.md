...
版本：**v1.3.2**（2026-09-22；v1.0 = 立卷，v1.1 = 值级对拍收口存档 + 引用索引补模板/覆盖率纪律，
...
v1.2.2 = **面二（产物/IR 面）落地** + 覆盖边界成要件 + 钉子 `gen_guard`，
v1.2.3 = **面三（覆盖率读数面）落地**（含"棘轮 ≠ 复核面"边界），
v1.2.4 = **面二边界收口**（IR 校验族 G1–G8/G12/G13 + G10/G11 装自报行 ⇒ 由"不覆盖"转"覆盖"），
v1.2.5 = **役 2 / 2a `ImpliedBy` 零机械转正** + R-19 阶段 0 两枚反例钉（引擎真值钉 / 模型侧分道钉）+ 覆盖率读数同笔复算 9937 → 9939，
v1.2.6 = **役 2 / 2b 恢复道入模 + `Caret` 转正**（声明门 `[type_mapping.recovery_close]` + 探针面同扩 D2；覆盖率 9939 → 9982、permille 906 → 907），
v1.2.7 = **N6 通道定性**（宿主 CI = 建木体系；无匿名读通道；仓库公开且推送已落地）+ §1.2 新增「主仓工具链弃用面」行（W18 主仓半余量 A 12 / B 132 入册），
v1.2.8 = **C-T12 结案**（复跑证伪：release 下 MoonBit 词法 33.84M tok/s vs C-FFI 15.42M；原 2.3–2.6× 系 debug 档口径）+ 性能分段现况（验证段占 81%）入册，
v1.3.0 = **面四（性能读数面）落地**（P0 性能役）：`perf-review.txt` + CI 步「复核面·性能」——**比较体只装机器无关相对判据**（抖动实测 ±15% 而判据全 true）+ 钉子第五钉，见 §7）
v1.3.1 = **面三覆盖率复核表同笔重生成（补漏）**：P4 词法两刀（字节类表 + IRI/PrefName 热循环局部化）入库时漏了
v1.3.2 = **面一（套件自报行面）同笔重生成（补漏）**：`src/gen_trig` 新增两行自报（简单/复杂语料）⇒
`suite-review.txt` **+2 行**（面二 `gen-review.txt` 零差）；沿革 = "同笔纪律漏跑"第四例
（面三复现 / 格式门 / `.mbti` 漂移 / 面一复现），四张判据各自兑现一次。
"改代码 ⇒ 同笔重生成面三"⇒ CI 冷跑复现门当场抓住（10115/11140 vs 冷跑 10135/11160，差 +20/+20 = 两刀新行）；
本笔补：**10135/11160**、permille **907 → 908** 同笔推高、三方言可达账不变。
...
| 子仓 | 默认臂计数 | `TODO(P0):` 计数（`model_exec.mbt`） | **trig 30 / n3v2 48**（与 CI 常数一致；R-17 转正 −1，剩 (BnpExpectPredicate, Id) 无 action 行） |
...
| 子仓 | 测试 | 本机 | **477/477**（含 `gen_guard` 四钉） |
...
**自报行（两族）**：① **产物黄金门**（3 行）② **IR 校验族**（11 行：G1–G8 / G12 compose 子机门负例 / G13 锚点漂移 /
**G10 族声明装配门 + G11 可达性门**——后两者随 `n3gen_build` 每次构建执行，构建成功即通过）。
**覆盖边界（W17 收口后）**：本面覆盖**产物 ≡ 表源**三类门 + **IR 校验族**；
**不覆盖**套件自报行面（面一）/ 覆盖率面（面三）/ 语义面 / 性能面 / **`gen_md` 产线自带 G0–G7 门**（另一模块）。
【沿革】W17（2026-09-20）**边界收口**：IR 校验族原先自报行未装、只能在边界里如实登记为"不覆盖"，本轮装 11 行后转为**覆盖**
（钉子防回退：`gen_guard` 断言表内 IR 行在册且边界不得再写"未装"）。
...
{
  echo "[行覆盖]"
  moon coverage report -f summary | tail -n 1
  echo "[可达命令名覆盖]"
  moon test 2>&1 | grep -o '可达覆盖账 [a-z0-9]*：分母 [0-9]* / 分子 [0-9]* / 缺口 [0-9]* / 不可达 [0-9]*' | sort -u
} > coverage-review.txt
```

**自报行**：行覆盖末行（`Total: <covered>/<total>`）+ 三方言台账行（分母 / 分子 / 缺口 / 不可达）——**原样收纳，零纸解析**。
**留证**：子仓 CI `static` 步「复核面·覆盖率（可复现性检查）」；本地复跑同命令即可复现（2026-09-20 冷口径两次逐字节一致）。
**机器门**：同命令重生成 ≡ 入库表（逐字节）+ **负控**（改一位即红，实测 ✓）。
**覆盖边界**：本面只覆盖**冷口径两量读数**；**不覆盖**其它 OS / 工具链版本 · **棘轮判定**（时间维度，归基线文件）· 面一 / 面二 · 未覆盖行的逐行解释（归解释表）。
**⚠ 纪律**：**读数随代码变化 ⇒ 改代码必须同笔重生成本表**（否则 CI 红）；**棘轮 ≈ 复核表：二者不可互替**（`const.md` §6.5.6）。

### 1.5 面四：性能读数面（**本轮落地**，P0 性能役）
...
**与前三面的本质差异（本面的设计要点）**：性能读数**有抖动、且随机型变化**（同日三次跑总耗时 7.14 / 8.36 / 7.83 ms，±15%）
⇒ **不能照抄"逐字节比对读数"**。故本表分两层：

- **比较体（参与 CI 逐字节比对）= 机器无关的相对判据**：段序（`验证 > 引擎 > 物化`、`验证 > 词法`）+
  档位带（`吞吐 > 0.5M quads/s`，用于把 debug 档挡在外面）+ **`词法(Lexermoon) > C-FFI(Lexerc)`**（C-T12 结案的常驻判据）。
- **留证读数（机器相关，只作展示，不进比较体）= 表头 `#` 注释 + CI `::notice::`**：分段 µs / 吞吐 / 双词法对照。

**判据（两条）**：① 比较体同命令重生成**逐字节可复现**；② 相对判据**全为 true**（false 即 diff 红）。
**留证**：子仓 CI `static` 步「复核面·性能（可复现性检查）」；本地复跑同命令即可复现（同日四次跑比较体逐字节一致）。
**机器门 + 负控**：改一位即红（diff 语义，实测 ✓）；表结构成要件由钉子 `gen_guard` 第五钉守（口径/命令/边界/两节比较体必在，
且**比较体内不得出现绝对读数**——`µs` 一律禁，含 `quads/s` 的行只许是阈值判据行）。
**覆盖边界（必写）**：**不覆盖** ① 其它机型 / 其它规模的绝对吞吐（机器相关，跨机不可比）；
② 与 Jena / Oxigraph / rdflib 的对拍（**语义强度不同**，见 `src/bench/README.md` 口径三条）；
③ **棘轮**（历史对比，时间维度，与复核面不可互替，§6.5.6）。
**⚠ 纪律**：改代码 / 改档位 ⇒ 同笔重跑生成命令并刷新本表（否则本步红）。
...
1. **N6 宿主 CI 观测**：**通道已定性（2026-09-21 匿名探测），仍差一次登录态观测**。
   已测得的事实：① 宿主 CI = GitLink 的**建木（Jianmu）**体系——仓库页内联
   `jianmu_devops_url = https://jianmu.gitlink.org.cn`；② GitLink 主站 **不是 Gitea API 兼容**
   （`/api/v1/version|repos|actions/*|badges/*` 全部回落 SPA HTML 或 JSON-404），Jianmu 前端亦为
   SPA（739B shell，资产在 `jianmu-app.assets.dghub.cn`）⇒ **无匿名读通道**；
   ③ 两仓**公开可读**：匿名 `git ls-remote https://gitlink.org.cn/thy7/mbt.ttl.git` 通，
   且远端 HEAD = 本地 `a1227f0` ⇒ **推送确已落地**（间接证据，非 run 状态）。
   缺什么：一次**登录态**观测（token 或浏览器）。**观测配方（交给有凭据者，5 分钟）**：
   登录 GitLink → 仓库 → 流水线/CI（建木面板）→ 记 run id + 提交 sha + 结论；若能开 DevTools
   抓一次面板 XHR endpoint，则把该 endpoint 记入本卷，日后可脚本化。
   **役 1 深挖（2026-09-21，仍无凭据）**：④ **平台 API 无 CI 面**——GitLink《API Reference》
   （`https://www.gitlink.org.cn/docs/api`，服务端渲染，**212 端点**）只覆盖仓库管理
   （files/pulls/tags/contributors/compare/…），**无任何流水线/构建/执行记录端点** ⇒ 无脚本化读法；
   ...
   **解锁动作（一步判定）**：登录后看仓库页有无「DevOps / 流水线 / 执行记录」入口——
   ...
   （或在平台侧确认是否支持 GitHub Actions 兼容），并把配置路径回灌本卷与 `world/todo.md`。
   本地对应（已具备）：两仓 `.github/workflows/test.yaml` 入库、口径与本地一致、§1 静态门全绿。
...
| 版本 | 日期 | 变更 |
| --- | --- | --- |
| v1.3.0 | 2026-09-22 | **面四（性能读数面）落地**（P0 性能役）：新表 `perf-review.txt`（子仓根）+ 子仓 CI 步「复核面·性能（可复现性检查）」+ `gen_guard` **第五钉**（口径/命令/边界/两节比较体成要件；比较体内禁绝对读数）。**设计要点（与前三面本质差异）**：性能读数抖动且跨机不可比（同日三次 7.14/8.36/7.83 ms，±15%）⇒ **比较体只装机器无关相对判据**（段序 `验证>引擎>物化`、`验证>词法`、档位带 `>0.5M quads/s`、**`词法 > C-FFI`**＝C-T12 常驻判据），**绝对读数只作留证**（表头 `#` + CI `::notice::`）。新增 §1.5 面四 + §1.2「性能面」行；§1.1 面一覆盖边界的"④ 性能面"改为指向面四。验收：本地同命令重生成比较体**逐字节一致**（四次跑实证）+ 负控改一位即红；子仓 **480/480**、`check --deny-warn` 0。 |
| v1.2.8 | 2026-09-21 | **C-T12 结案（方向反了）**：复跑 `moon test --target native --release src/gen_nquads -f "*词法役 P1*"` ⇒ 10k `Lexermoon` **33.84M tok/s** vs `Lexerc`(C FFI) **15.42M** ⇒ C-FFI 慢 **2.20×**（1k 1.82×）；原"MoonBit 落后 2.3–2.6×"出自 **debug 档**、且倍数属 C-FFI 路径。§1.2 C-T12 行 → `[已结案]`（附复跑命令 + 重评估条件"若改每 token 跨界口径"）。**性能分段现况**（10k / native release，同日复跑）：总 **7.14 ms**，其中 **验证段 5.80 ms（≈81%）** / 引擎 3.14 / 词法 1.47（21%）/ 物化 1.34 ⇒ 性能真战场 = 验证段。**W18 补洞**：子仓 **native 档**诊断 1 处（`engine_c.mbt` 的 `Lexerc`，native-only 文件）已清 ⇒ **按档跑口径**入册（wasm 绿 ≠ native 绿）。 |
| v1.2.7 | 2026-09-21 | **N6 通道定性（仍未闭，但阻塞点精确化）**：宿主 CI = GitLink **建木（Jianmu）**体系（仓库页 `jianmu_devops_url`）；主站非 Gitea API 兼容（`/api/v1/*` 全回落 SPA/JSON-404）、Jianmu 前端亦 SPA ⇒ **无匿名读通道**；两仓匿名 `ls-remote` 通且远端 HEAD = `a1227f0` ⇒ **推送已落地**（间接证据）。观测配方写入 §5（登录态 5 分钟，含可脚本化的 endpoint 抓捕建议）。§1.2 新增 **「主仓工具链弃用面」** 行：子仓 0（86/86 清 + 钉版 20260920）；主仓 **A 12 + B 132**（B 全在 `*_test.mbt`）⇒ 主仓钉版**故意留 20260915**，跟升与清偿同笔。 |
| v1.2.6 | 2026-09-21 | **役 2 / 2b：恢复道入模 + `Caret` 转正**。① **声明门**（生成器）：`[type_mapping.recovery_close]`（`Dot = "All"` / `EOF = "SPO"`）⇒ 生成 `Model.recovering` + `Response::Skip` + `recovery_enter`（清 `frames`/`is_src`，**不清 path 三账**）+ `recovery_swallow`（窗内进表外吞咽、收窗清槽归位 `ExpectSubject`）+ **`model_execute` 外层包装**（窗口落在 `model_execute` 而非只挂 `transition`——手写探针全走前者，挂后者会让值级门对恢复道全盲，实证见下）+ label 补 `Skip` 臂；未声明路径**零字节**（生成器门测试在册）。② **探针面同扩（D2）**：`N3Probe` 增恢复窗旗 + 错误即清账半边，窗内调**引擎唯一实现** `N3Context::recover_consume` 并返空批；`system.mbt` 空批 ⇒ `Skip`。③ **反例钉翻转**：`r19_probe_wbtest.mbt` [5..9] 由 `Error(0)` 改 `Skip`、[10] 收窗点也 `Skip`（**不发空槽 quad**）——**负控实测**：只改模型未扩探针时该钉恰在 [5] 处红（`Error(0) != Skip`）。④ `Caret` 权重 0→1（`business.mbt` 同笔再生恰 1 行）。⑤ 覆盖率同笔复算 **9939 → 9982**（分母 10965 → 10998，permille **906 → 907** 同笔推高）+ 面三表重生成。门况：子仓 **479/479**、普查 2255 债 0/违例 0、数据面 314/136 零分道、trig 1224 对 0 分道。 |
| v1.2.5 | 2026-09-21 | **役 2 / 2a `ImpliedBy` 零机械转正**（IR 全 5 行 = `set_predicate`，与已转正的 `Implies` 逐行同形 ⇒ 无反向旗、不涉恢复道）：TOML 权重 0→1 + `business.mbt` 同笔再生（**恰 1 行**）+ `metrics.mbt` 镜像钉表同笔；§V.4 两条确认（确定性复跑无新分道 + 权重动作可观测：seed 2026 分布命中 `ImpliedBy=1`）；三种子（12345/1/2026）随机门绿。**R-19 阶段 0 两枚反例钉**：引擎侧真值钉（`engine_wbtest.mbt`：墙后 5 事件被吞、零槽位出现）+ 模型侧分道钉（`quicktest/r19_probe_wbtest.mbt`：探针⟷模型逐步全同，分道点 = 墙后第一个事件，**2b 同笔翻转**）。§1.2「权 0 事件」行改 `Caret`/`EOF` 两件；覆盖率同笔复算 **9937 → 9939**（分母 10965 不变，permille 906 保持）+ 面三表同笔重生成。**另账**：工具链口径漂移（本地 20260920 vs CI 钉 20260915）入册为**账非债**（世界卷 R30 / P2-8，待拍钉版）。 |
| v1.2.4 | 2026-09-20 | **面二边界收口（W17）**：给 IR 校验族装 **11 行自报行**（G1–G8 / G12 / G13 / G10+G11，其中 G10/G11 随 `n3gen_build` 执行）⇒ 面二由"产物 ≡ 表源 三类门"扩为**两族**（产物 + IR 校验）；`gen-review.txt` 表头边界节把该族从**不覆盖**移入**覆盖**并留沿革；§1.2「产物/IR 面」行同步；钉子 `gen_guard` 加防回退断言（表内 IR 行在册 + 边界不得再写"未装"）。 |
| v1.2.3 | 2026-09-20 | **面三（覆盖率读数面）落地**（随世界宪法 **v3.2**）：新表 `coverage-review.txt`（子仓根；冷口径两量原样收纳）+ 子仓 CI「复核面·覆盖率（可复现性检查）」（负控实测有牙）；新增 **§1.4 面三**（复现命令 / 留证 / 边界 / **"棘轮 ≠ 复核面"** 纪律）；§1.2 加「覆盖率面」行；钉子扩面三边界（`gen_guard` 四钉）。 |
