...
版本：**v1.1.6**（2026-09-20；v1.0 = 立卷，v1.1 = 值级对拍收口存档 + 引用索引补模板/覆盖率纪律，
...
| N8 | **主仓测试套件偶发红**（2026-09-19 观测 **3 次** `287 → 286`；第 3 次由捕获循环首轮复现并**留全量日志**）——**已定案**：非原子写盘（`src/rdf/domain_to_ir.mbt:1950-1953`，truncate-then-write）× 同套件并发读（`src/rdf/trig_domain_toml_gen.mbt:236` 逐字节比对）⇒ 读到**空文件** | → **转立案**（去向 = `src/quick_machine/todo.md`「立案：非原子写盘 × 同套件并发读」条目；处方 = 写盘原子化 + 并发读隔离）；该发现的红线已回灌世界宪法 **§6.4.6** |
...
2. **役 15 余量**：深片排除项的权重放宽（表 / IR 侧），不属于门缺陷。
   ...
   **重评估条件**：深片缺口再现时重看。
3. **N8 主仓套件偶发红（已定案 → 转立案）**：症状 = 偶发 `286/287`（观测 3 次，第 3 次由捕获循环首轮复现）。
   **根因（全量日志实证）**：`src/rdf/domain_to_ir.mbt:1950-1953` 的物化 `async test` 用
   **truncate-then-write** 写 `src/rdf/fsm_out/trig_fsm.toml`，与同套件 `src/rdf/trig_domain_toml_gen.mbt:236`
   的**逐字节比对**并发 ⇒ 读者落进"已 truncate、未写完"窗口，读到**空文件**（失败消息末尾 `!= ""`）。
   已做到哪：捕获命令固化为写日志的 10 轮循环（首轮即抓到全量输出）；写者/读者两侧定位到行。
   下一步：**转立案**（去向 = `src/quick_machine/todo.md`「立案：非原子写盘 × 同套件并发读」）——
   处方 ①**写盘原子化**（临时文件 + 原子 rename，对全体读者免疫）②并发读隔离（读 fixture / 该组串行）。
   **重评估条件 / 结案判据**：修完后**捕获循环 30 轮（含冷跑 3 次）全绿**结案；再出现即回看本条。
   红线已回灌：世界宪法 **§6.4.6「生成物写盘必须原子」**（同笔 v2.7）。
