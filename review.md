...
版本：**v1.2.5**（2026-09-21；v1.0 = 立卷，v1.1 = 值级对拍收口存档 + 引用索引补模板/覆盖率纪律，
...
v1.2.5 = **役 2 / 2a `ImpliedBy` 零机械转正** + R-19 阶段 0 两枚反例钉（引擎真值钉 / 模型侧分道钉）+ 覆盖率读数同笔复算 9937 → 9939，见 §7）
...
| 权 0 事件（`Caret`/`EOF`） | quicktest 权重 0 | `[立案]`×1 + `[设计]`×1 | `Caret`=**R-19 恢复道**（役 2 在办：D1 入模 / D2 探针同扩；阶段 0 已固化两枚反例钉）；`EOF`=不可达（反断言钉）。沿革：`Id` 随 R-17（2026-09-20）转正、`ImpliedBy` 随役 2 / 2a（2026-09-21）**零机械转正**（IR 全 5 行 = `set_predicate`，与 `Implies` 逐行同形） | `Caret` 恢复位落地时 | `src/quick_machine/todo.md` P4.6 + `src/ttl/todo.md` §AP |
